...
bool web::Init()
{
	if( pEthernet == 0 )
	{
		TRACE_CRIT(F("Cannot start listening on WEB port!\n"));
		return false;
	}
	m_server = pEthernet;
	return true;
...
void ServeHeader(FILE * stream_file, int code, const char * pReason, bool cache, const char * type)
{

	//TRACE_VERBOSE(F("ServeHeader - Reason:%s, type:%s\n"), pReason, type);
...
void ServeFile(FILE * stream_file, const char * fname, SdFile & theFile, EthernetClient & client)
{
	...
	//TRACE_VERBOSE(F("ServeFile - ext:%s\n"), ext);
	...
	if (ext > fname)
	{
		...
		else if ( (strcmp_P(ext, PSTR("log")) == 0) || (strcmp_P(ext, PSTR("LOG")) == 0) ||	// logs
				  (strcmp_P(ext, PSTR("det")) == 0) || (strcmp_P(ext, PSTR("DET")) == 0) ||	// and watering schedules/logs
				  (strcmp_P(ext, PSTR("sch")) == 0) || (strcmp_P(ext, PSTR("SCH")) == 0) )	//
			ServeHeader(stream_file, 200, PSTR("OK"), false, PSTR("text/plain"));

		else if ( ext[0] >= '0' && ext[0] <= '9')											// metrics are stored in files with numeric extensions
			ServeHeader(stream_file, 200, PSTR("OK"), false, PSTR("text/plain"));
		else
			ServeHeader(stream_file, 200, PSTR("OK"), true);
	}
	...
	{
		int bytes = theFile.read(sendbuf, sizeof(sendbuf));
		if (bytes <= 0)
			break;
...
void web::ProcessWebClients()
{
	...
	if (client)
	{
		...
		FILE * pFile = fdopen(client.GetSocket(), "w");
		 ...
		 {
			SYSEVT_ERROR(F("ProcessWebClients- ERROR parsing request!"));
			ServeError(pFile);
		 }
		 else
		 {
...
// This is the "catch all" case, that also serves static HTML files, *.js etc.
			{
				...
				// prepend path
				memmove(sPage + 6, sPage, sizeof(sPage) - 6);
				memcpy(sPage, "./web/", 6);
