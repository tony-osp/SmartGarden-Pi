...
uint32_t EthernetServer::localIP()
{
	return 0;
}

uint32_t  EthernetServer::gatewayIP()
{
	return 0;
}
...
bool SdFile::open(const char* path, uint32_t oflag)
{
	char open_mode[5];

	if( oflag & O_CREAT ){
		if( (oflag & O_WRITE) && (oflag & O_APPEND) )strcpy( open_mode, "a+" );
		else if( oflag & O_WRITE )					 strcpy( open_mode, "w+" );
	}
	else {
		if( (oflag & O_WRITE) && (oflag & O_APPEND) ){  // open existing file for append, but fail if the file does not exist.
														// standard C++ library does not support it, have to emulate the behavior
			FILE *fid = fopen(path, "r");
			if( fid == 0 ) return false;

			// OK, file does exist. Close it and re-open in append mode
			fclose(fid);
			strcpy( open_mode, "a+" );
		}
		else {		// default is open for read
			strcpy( open_mode, "r" );
		}
	}
	
	m_fid = fopen(path, open_mode);
	return m_fid != 0;
}
...
inline char *sg_fgets(char *buf, int buflen, FILE *file)
{
	return fgets(buf, buflen, file);
}

int SdFile::fgets(char *buf, size_t buflen)
{
	char *strp = sg_fgets(buf, int(buflen), m_fid);
...
{
	return fputc(c, m_fid);
}

int SdFile::write(const char *buf, size_t buflen)
{
	return fwrite(buf, 1, buflen, m_fid);
}


int SdFile::print(const char *str)
{
	return fprintf(m_fid, "%s", str);
}

int SdFile::println(const char *str)
{
	return fprintf(m_fid, "%s\r\n", str);
}
...
inline int sg_mkdir(const char *str)
{
	return mkdir(str, 0777);
}

bool SdFat::mkdir(const char *str)
{
	if( sg_mkdir(str) != -1 ) return true;
	else					  return false;
}
...
bool SerialClass::begin(int speed)
{
	port = serialOpen ("/dev/ttyAMA0", speed);
...
{
	tcdrain(port);	// note: I'm using POSIX tcdrain() call for fflush - like behavior. 
					//       Unfortunately serialFlush() call in WiringPi discards buffers rather than waiting for completion
}
