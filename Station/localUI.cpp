...
// Data members
uint8_t OSLocalUI::osUI_State = OSUI_STATE_UNDEFINED;
uint8_t OSLocalUI::osUI_Mode  = OSUI_MODE_UNDEFINED;
uint8_t OSLocalUI::osUI_Page  = OSUI_PAGE_UNDEFINED;
uint8_t OSLocalUI::display_board = 0;                                                      // currently displayed board


#ifdef USE_I2C_LCD
ST7036 OSLocalUI::lcd( LOCAL_UI_LCD_Y, LOCAL_UI_LCD_X, 0x78 );
#else
// and this is controller with big LCD version
...
uint8_t GetNextStation(uint8_t curr_station);
uint8_t GetPrevStation(uint8_t curr_station);

uint8_t get_button_async(uint8_t mode);

uint8_t weekday_today(void);
int get_key(unsigned int input);
void buttons_loop(void);
void manual_station_on(uint8_t sid, int ontimer);


// button functions

uint8_t button_read_busy(uint8_t pin_butt, uint8_t waitmode, uint8_t butt, uint8_t is_holding);        // Wait for button
uint8_t button_read(uint8_t waitmode);                                                                         // Read button and returns button value 'OR'ed with flag bits
...
//
uint8_t OSLocalUI::begin(void)
{
#ifdef USE_I2C_LCD
     lcd.init();         // I2C library uses different init conventions
	 lcd.clear();
#else
	 lcd = new LiquidCrystal(PIN_LCD_RS, PIN_LCD_EN, PIN_LCD_D4, PIN_LCD_D5, PIN_LCD_D6, PIN_LCD_D7);
...
// RPi vs ARDUINO compilers use slightly different convention for binary literals
#ifdef ARDUINO
    uint8_t lcd_custom_char[8] = {
      B00000,
      B10100,
      B01000,
      B10101,
...
// local UI is enabled, call appropriate handler
  ...
  // Force UI refresh.
uint8_t OSLocalUI::refresh(void)
{
   return callHandler(1);
}

uint8_t OSLocalUI::callHandler(uint8_t needs_refresh)
{
...
// Stop UI updates and input handling (useful when taking over the screen for custom output)
  uint8_t OSLocalUI::suspend(void)
  {
        osUI_State = OSUI_STATE_SUSPENDED;
...
// Switch UI to desired Mode
  uint8_t OSLocalUI::set_mode(char mode)
  {
        osUI_Mode = mode;
        osUI_Page = 0;
 ...
 //
 uint8_t OSLocalUI::modeHandler_Home(uint8_t forceRefresh)
 {
...
uint8_t OSLocalUI::modeHandler_Manual(uint8_t forceRefresh)
{
  static uint8_t sel_manual_ch = 0;
  static unsigned long old_millis = 0;
  static uint8_t man_state = 0;                    // this flag indicates the UI state within MANUAL mode. Valid states are:
                                                                        // 0 - initial screen, select the channel
                                                                        // 1 - entering number of minutes to run
  static uint8_t num_min = 0;
  static uint8_t max_ch = 0;
...
//					manual_station_on((uint8_t)(sel_manual_ch+sStation.startZone), num_min);        // start required station in manual mode for num_min only if required time is != 0
                }
...
*/
uint8_t OSLocalUI::modeHandler_Status(uint8_t forceRefresh)
{
// assert
...
//   if( forceRefresh != 0 ){   // entering VIEWCONF mode, refresh things
   if( 1 ){   // always refresh
          ...
          if( osUI_Page == 0 ){
             ...
             uint32_t ip = pEthernet->localIP();
			 LCD_SETCURSOR(lcd, 0, 1);
             lcd_print_ip((uint8_t *)&ip);
...
#if HW_ENABLE_ETHERNET
			 LCD_SETCURSOR(lcd, 0, 1);
             uint32_t ip = pEthernet->gatewayIP();
             lcd_print_ip((uint8_t *)&ip);
#else // HW_ENABLE_ETHERNET
             lcd_print_line_clear_pgm(PSTR("No Ethernet"), 1);
#endif // HW_ENABLE_ETHERNET
...
#ifndef HW_ENABLE_SD
uint8_t OSLocalUI::modeHandler_Setup(uint8_t forceRefresh)
{
  static unsigned long old_millis = 0;
  static uint16_t	curr_value;			// use 16bit variable to avoid problems with edge conditions
  static uint8_t		state = 0;              // this flag indicates the UI state within Setup mode. Valid states are:
...
void OSLocalUI::lcd_print_station(char def_c, uint8_t sel_stn, char sel_c, uint8_t max_ch) {
...
void OSLocalUI::lcd_print_pgm(const prog_char * str) {
  uint8_t c;
  while((c=pgm_read_uint8_t(str++))!= '\0') {
    lcd->print((char)c);
  }
}

// Print a program memory string to a given line with clearing
void OSLocalUI::lcd_print_line_clear_pgm(const prog_char * str, uint8_t line) {
...
void OSLocalUI::lcd_print_time(uint8_t line)
{
  time_t t=now();
  LCD_SETCURSOR(lcd, 0, line);
...
// Print free memory
void OSLocalUI::lcd_print_memory(uint8_t line)
{
  LCD_SETCURSOR(lcd, 0, line);
...
// print ip address
void OSLocalUI::lcd_print_ip(const uint8_t *ip) {
  uint8_t i;
...
uint8_t weekday_today() {
  return ((uint8_t)weekday(now())+5)%7; // Time::weekday() assumes Sunday is 1
}
